...
def _build_identity_preamble(
    ...
    *,
    ...
    return (
        ...
        f"emit a response with NO tool_use blocks (just final text, or "
        f"empty). That's it — no action required, no terminal tool. As "
        f"long as you keep calling tools, the wakeup keeps running.\n"
        f"\n"
        ...
        f"  • Wait for handed-off work → `await_subagents()` (it yields "
        f"the wakeup; you'll be re-woken when the children finish)\n"
        ...
        f"  1. **Your own sent mail** — `mailbox_read(box=\"sent\")` "
        f"shows what you said. Treat every owner/peer reply as a "
        f"durable record of \"what I committed to\". Before saying "
        f"\"I'll do X\", check sent for prior commitments on the "
        f"same topic so you don't re-promise.\n"
        ...
        f"  3. **Notes file** — a pre-created markdown file lives at "
        f"`{notes_path}`. It is YOURS. Append to it whenever you "
        f"learn / decide / promise something that should outlive this "
        f"wakeup. Examples: open threads, owner preferences, pending "
        f"decisions, gotchas. Read it at wakeup start when you need "
        f"context:\n"
        f"      • read:   `read_memory(\"facts/agent-{agent_id}-notes.md\")`\n"
        f"      • append: `shell_exec(\"cat >> {notes_path} <<'EOF'\\n"
        f"...\\nEOF\")` (or python_exec with `open(..., 'a')`)\n"
        f"  4. **report_progress checkpoint** — for crash recovery "
        ...
        f"You don't need to use all four every wakeup — pick whichever "
        f"matches the durability you want. But if a thought matters "
        f"beyond this wakeup, one of them must happen.\n"
        f"\n"
        ...
        f"can dispatch more, or `await_subagents()` to wait, or simply "
        f"stop calling tools and let the scheduler wake you when the "
        f"subagent reports back. Three invariants:\n"
        ...
        f"  • **Track delegated work in your notes.** Append the "
        f"task_id, who you delegated to, and what's expected back to "
        f"`{notes_path}`. When the subagent's reply arrives in a "
        f"future wakeup you'll have the trail.\n"
        f"\n"
