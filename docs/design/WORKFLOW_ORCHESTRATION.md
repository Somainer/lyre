...
> **状态**：**PR1 + PR2 + PR3 + PR4（ephemeral 回收）+ PR4b（supervisor 重启/强度/升级）已落地**（见 §13）。其余 PR4c（预留单例槽 + Phase 2 SIGKILL-loop bound）/ PR5 / PR6 待实现。
>
...
| **PR4c** | 加固:预留单例槽(`_available_slots` 给 NULL-parent 单例留一槽,§3 starvation 防护)+ Phase 2 对 ephemeral 子的真-SIGKILL crash-loop 强度 bound(租约恢复也计强度,超限改为升级而非重跑) | 预留槽使 dispatcher 饱和下仍可运行 / Phase2 恢复计强度 / 跨层 kill(成员+ephemeral) |
