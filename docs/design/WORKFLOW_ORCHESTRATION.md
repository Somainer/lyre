...
> **状态**：**PR1（`needs_input` park/resume + `repos.transaction()`）+ PR2（mailbox 驱动 barrier）+ PR3（`task_terminated` 邮件 / OTP monitor）+ PR4（ephemeral agent 回收 reaper）已落地**（见 §13）。其余 PR4b（重启 + 强度 + 升级 + 预留槽）/ PR5 / PR6 待实现。
>
> **PR4 范围说明**：原 §5 "Hybrid Reaper" 把回收 + 一对一重启 + 重启强度 + 升级 + 预留单例槽打包成一个 PR4。落地时拆分:**PR4 = 纯回收**（owner 的明确诉求"回收 ephemeral agent"，Erlang 作参照）——`supervision.ephemeral` 标记的 agent 跑完任务（有过任务且无在飞任务）后由调度器 Phase 0.8 reaper 自动 archive；失败/完成信号已由 PR3 `task_terminated` 承载,故回收是纯 GC（不发信、无新表）。**重启 / 重启强度（`supervision_state`）/ 升级邮件 / 预留单例槽 → PR4b**（韧性层,需要 §3 的 starvation 防护与 §5 的 `bump_and_check_intensity`）。
>
...
| **PR4b** | 韧性层:`supervision_state`(重启强度滑窗)、一对一 `bump_and_check_intensity` 重启、超限升级邮件(high,LLM 入口)、Phase 2 恢复也计强度、预留单例槽(`_available_slots` 给 NULL-parent 单例留一槽,§3 starvation 防护)、reaper 改为"该重启则重启,否则回收" | 风暴上限+升级 / 超限子被回收不悬空 / Phase2 恢复计强度 / 预留槽使 dispatcher 饱和下仍可运行 / 跨层 kill(成员+ephemeral) |
