...
> **状态**：**PR1 + PR2 + PR3 + PR4（ephemeral 回收）+ PR4b（supervisor 重启/强度/升级）+ PR4c（加固：预留单例槽 + Phase 2 crash-loop bound）+ PR5（协调器聚合 + 升级处理 + persona 文档）+ PR7（idle-reclaim：Dispatcher 驱动回收非 ephemeral 闲置子）已落地**（见 §13）。其余 PR6（TTL 强关 + 可观测性）待实现。整个 §5 "Hybrid Reaper" 韧性层至此完整。
>
