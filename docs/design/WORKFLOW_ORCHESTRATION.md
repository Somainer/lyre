...
> **状态**：**全部落地 —— PR1 + PR2 + PR3 + PR4（ephemeral 回收）+ PR4b（supervisor 重启/强度/升级）+ PR4c（加固：预留单例槽 + Phase 2 crash-loop bound）+ PR5（协调器聚合 + 升级处理 + persona 文档）+ PR6（fan-in 全局 TTL + 归档可观测性）+ PR7（idle-reclaim：Dispatcher 驱动回收非 ephemeral 闲置子）**（见 §13）。整个 workflow-orchestration 设计 + §5 "Hybrid Reaper" 韧性层至此完整。
>
...
- **不新增发送工具**：子用既有 `mailbox_send(result_for=G, leg_key=k)`。`_mailbox_send` 内、入 outbox 前：(a) 载组、(b) 仅 cancelled 拒绝（晚到结果放行——rejecting late results to a resolved group created an unbounded leg-restart burn loop, fixed in PR #59）、(c) **血缘校验**（发送者须在名册里拥有 `(group, leg_key)`，防 CLI 伪造）、(d) `jsonschema` 校验 payload、(e) 强制 `recipient=coordinator` 并盖 `metadata.fan_in`。所有校验在入 outbox 前 `ToolError`（fail-closed）。判官投票是一等结果邮件（`metadata.fan_in.verdict`），**不用 `mailbox_react`**（它不产生 `mailbox_messages` 行、不可计数）。
...
| 工具 | 状态 | 行为 | allowlist |
|---|---|---|---|
| `mailbox_send` | 改 | 加 `result_for/leg_key`；设了则载组→仅 cancelled 拒（PR #59）→血缘校验→`jsonschema` 校验→强制 recipient=协调器→盖 `metadata.fan_in`，全部入 outbox 前 `ToolError`（fail-closed） | 全体（既有） |
...
| **PR4** ✅ **已落地** | ephemeral 回收 reaper：`create_agent` 加 `supervision={ephemeral}` 参数(写 `agents.metadata.supervision`,OTP child_spec);`AgentRepository.find_reapable_ephemerals`(ephemeral + 非 archived + parent 非 NULL + 有过任务 + 无在飞任务;liveness 键于在飞 TASK 状态,孤儿 wakeup 不挡);调度器 **Phase 0.8** `_reap_ephemeral_agents` 幂等 archive,无邮件(GC,失败/完成由 PR3 承载) | done 任务→回收 / 在飞任务→不回收 / 零任务→不回收(create→dispatch 竞态守卫) / 非 ephemeral→永不回收 / NULL-parent→不回收 / 孤儿 wakeup 不挡回收 / 幂等 / `_tick` 集成 / `create_agent` 存储+校验 supervision,共 9 个，全绿 |
| **PR4b** ✅ **已落地** | 韧性核心:`supervision_state` 类型化重启强度滑窗(`bump_and_check_intensity` 原子读改写,torn-write 安全)、reaper 升级为"按 restart 策略(temporary/transient/permanent)对 latest 结果决定 重启 vs 升级 vs 回收"、一对一重启(同 agent + 同 goal/metadata,fan-in leg 重试)、超限→升级邮件(high,LLM 入口)+ 回收、temporary 失败子→失败通知(不丢失)、ephemeral 的 PR3 `task_terminated` 抑制(reaper 接管)、每 agent 决策一个事务原子、`find_latest_task_for_agent`(按 rowid 取最新) | transient 重启失败/不重启完成 / permanent 重启完成 / temporary 失败→失败通知+回收 / 重启保留 fan-in metadata / 超限→升级+回收+`escalated_at` / 强度滑窗(含窗口重置,受控 `now`) / ephemeral task_terminated 被抑制,共 8 个，全绿 |
...
| **PR5** ✅ **已落地** | 协调器聚合 + 升级处理 + persona 文档。**一次性聚合原语**:`MailboxRepository.read_fan_in_results(recipient, group_id)`(每 leg_key 取最新 result-mail,`MAX(id) GROUP BY leg_key` 去重) + `fan_in_results(group_id)` 工具——返回 `{status, delivered, results:[{leg_key, from, result}], missing_legs}`,协调器被 ready 邮件唤醒后**一调到位**,不用手翻 inbox 里 low-urgency 的结果邮件。fan_in_open/status/results/cancel **接入 dispatcher allowlist**。**persona 文档**:dispatcher.md 加【fan-in barrier】(何时用 barrier、open→dispatch N 腿→停→ready 邮件→`fan_in_results`→综合→报 owner)+【处理升级】(对 `system:supervisor` 邮件按 `metadata.kind` ∈ {supervision_escalation / supervision_failure / task_terminated} 决策 re-plan/re-spec/换 agent 重派/放弃,真要 owner 决策才转发)。升级邮件契约由 PR4b 提供(high + 确定性 `external_id` ⇒ 每 agent 至多一封),PR5 锁回归 | fan_in_results 返回 typed 结果/缺腿 / read_fan_in_results 去重(latest wins) / 未知 group→error / 升级邮件 high + `metadata.kind=supervision_escalation` + 每 agent 一封(去重) / dispatcher allowlist 含 fan_in_* / fan_in_results 已注册,共 7 个,全绿 |
| **PR6** ✅ **已落地** | **(a) fan-in 全局 TTL**:`LYRE_FANIN_MAX_AGE`(config `[scheduler] fanin_max_age_s`,env-beats-toml,默认 0=关)——Phase 0.5 在既有 per-group `deadline` 之外,把**超全局上限的 open 组**强制 `expired`(now − `created_at` > cap;`set_status(guard='open')` 单赢家;ready 邮件带 `trigger` ∈ {quorum,deadline,ttl})。修了 `_row_to_group` 漏读 `created_at`。per-group deadline 仍是常开活性,TTL 是运营天花板。**(b) 归档可观测性**:`agents.archive_reason` 列(自由文本:reaped / storm_halted / idle_reclaimed / manual),`archive(agent_id, reason=)` **与归档同事务原子写**;reaper 干净/失败回收→`reaped`、超强度升级→`storm_halted`、Phase 2 crash-loop→`storm_halted`、`archive_agent` 工具→`idle_reclaimed`(dispatcher 收 stale 时传)/`manual`(默认)、CLI→`manual`;`unarchive` 清空。`list_agents` 透出 `archive_reason`+`archived_at`;dashboard Archived 表加 reason 徽章(storm_halted 红)。设计文档原提的 `reaped_at` 由更通用的 `archive_reason` 取代。 | TTL 关旧 open 组(deadline 未到也关)+ trigger=ttl / 默认关时不动(deadline 仍生效,trigger=deadline)/ TTL 幂等一封 / archive 记 reason + unarchive 清空 / reaper→reaped / Phase2→storm_halted / 工具→idle_reclaimed+manual / list_agents 透出 / config env-beats-toml + garbage/负数→0,共 12 个,全绿 |
