...
- **T4** 有界自邮件循环：`mailbox_send` 的 `deliver_at/deliver_in/recur_*` 参数把邮件
  ...
  §7「邮箱工具越权」行）。跨 wakeup 无进展闸 H2 设计已定稿但**尚未实现**（§7「H2 无进展闸」行）。
...
| fan-in 烧钱循环 | 组 resolve 后晚到结果被拒 → 干完活的 leg 被 O2 记 failed → transient 重启 → 再晚到，循环；60s 滑窗强度限制对分钟级失败永不触发；重启非 one-for-one（丢 git_context/tier_overrides/lease/deadline）；O1 哨兵先于重启决策插入 | `scheduler.py` Phase 0.5/0.8、`tools/mailbox.py` result 校验 | 待修（最高优先级，与 H2 同批） |
...
| H2 无进展闸 | 跨 wakeup 无进展即停的设计已定稿（LONG_RUNNING_ROBUSTNESS_3 §9）、零实现——"无预算、靠停"约束的最后缺口 | `scheduled_mail` + Phase -1 | 待做（下一批） |
