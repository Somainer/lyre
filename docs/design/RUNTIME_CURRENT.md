...
> built (2026-06). Where the foundation-era docs (FOUNDATION /
...
这个门有一个已知跨 tick 窗口，见 §7「双 wakeup 窗口」行）。
...
1. **孤儿清扫** — `close_orphans_for_task(task_id)`：上一次尝试可能在写入 wakeup 行后
   ...
   其他 task）。
2. **原子 start+claim**（scheduler.py ~1560）— `wakeups.start`（INSERT 行）与
   ...
   已不存在：要么什么都没发生（下个 tick 重试），要么行+租约
   同时存在（之后崩溃走普通租约恢复）。
3. **worktree + git 供给** — 每个 wakeup 都有一个 worktree（空 tmpdir，
   ...
   task_terminated 邮件且 wakeup 行未关，见 §7「失败路径静默」行）。
...
- **自动压缩**：本 turn `input_tokens ≥ compact_threshold(默认 0.7) × context_window`
  ...
  **同模型** LLM 摘要（失败 → 原始 trace 降级并打 `summary_degraded` 标）；产物标
  ...
  silent_close 收尾（已知问题：道歉信可能失实且 task 记 completed，见 §7「压缩谎报已发」行）。
...
`identity.agent_notes_rel_path`（flatten 的唯一实现，spawned `persona/name`
agent 不再写进无人读的子目录）；写入走 `fsutil.atomic_write_text`
（mkstemp+fsync+replace，全库唯一的原子写原语）。超过 `notes_max_entries`
...
- `lyre serve` 下 scheduler / outbox dispatcher / channels / dashboard 共享**一条**
  ...
  守 commit 边界不守语句执行，见 §7「共享连接回滚」行）。
...
异常处理粒度（F2 起三层）：`run()` 在 tick 级兜底；`_tick` 内 Phase -1/0.5/0/0.7/0.8
各自被 `_guarded_phase` 包裹（任一 phase 失败不再阻断其后的派发 Phase 2/3）；
Phase -1/0.5/0 内部再做**逐行隔离**——毒行单独失败并记账：`scheduled_mail` 连续
3 次投递失败 → 持久 `status='quarantined'` + 通知创建者（`delivery_failure_count`
列）；fan-in group 连续 3 次解析失败 → 经既有 `expired` 终态强制收场 + 通知
coordinator（内存计数器，重启清零、毒组会再次触发）；Phase 0 单 agent 失败仅记日志
（下 tick 自然重试，不阻塞他人）。
...
- **T4** 有界自邮件循环：`mailbox_send` 的 `deliver_at/deliver_in/recur_*` 参数把邮件
  ...
  high 注明最后一次并停止续期。**H2 无进展闸**（PR #59）：`max_no_progress` 设定后，
  Phase -1 在每次投递前评估主线自上一 occurrence 以来的进展——有外向产出（非自
  thread 邮件 / task 创建·完成·checkpoint）归零；最重单个 wakeup 超工作量地板且零
  产出记一轮空转；低于地板的等待心跳不计。连续 `max_no_progress` 轮空转即末次投递
  （同 T4 停-loop 路径）。设计稿里的 token 预算**未实现且已删**（reserved 的
  `budget_tokens`/`dry_round` 列已作为死代码移除——真要做须按新 feature 重建）。
  `list_scheduled_mail`/`cancel_scheduled_mail` 管理（已知所有权缺口
  §7「邮箱工具越权」行）。
...
| scratchpad | `~/.lyre/memory/scratchpad/<flat-id>.md` | agent 经 `update_scratchpad`（`append` 默认 / `overwrite` 整理） | 短期工作记忆；identity preamble 教 agent 醒来先读；原子写 |
...
| skills | `~/.lyre/skills/{approved,proposed,archived}/<name>/SKILL.md`（**目录形**，flat 文件不被识别）+ 内置库 `src/lyre/data/skills/`（运行时直读，不拷贝） | 固化流：worker 写 `proposed/`，reviewer 审后 FS mv 到 `approved/`（CAPABILITY_DISCOVERY / BUILTIN_SKILLS） | prompt 注入收叠的 name+description；同名 first-wins，用户 approved 覆盖 builtin；加载诊断有算无报（§7「skill 加载诊断」行） |
...
2. **persona model_preference 是必填项**：`tier`（flagship/workhorse/cheap）+
   ...
   **文档曾承诺的 LYRE_DEFAULT_MODEL fallback 不存在**（该 env var 被解析但无消费者，实现或删除待决）。
...
2026-06 深度 review 确认的缺陷中尚未修复的部分，自包含一行表（行名即正文引用锚点）。
已修复项不再列出，本文档正文即修复后现状。

| 行名 | 陈述 | 位置 | 现状 |
|---|---|---|---|
| 双 wakeup 窗口 | subprocess 模式下子进程写 wakeup 行慢于 1s tick，同 agent 可被双派发；Phase 2 恢复路径无 per-agent 门 | `scheduler.py` Phase 2/3 门 | 待修 |
| fan-in 重启遗留 | 重启非 one-for-one（丢 git_context/tier_overrides/lease/deadline）；O1 哨兵先于重启决策插入、roster 不随重启更新（烧钱循环本体已修，PR #59：晚到结果放行 + 终身重启上限） | `scheduler.py` Phase 0.5/0.8 | 待修 |
| 失败路径静默 | git 供给失败 / persona 缺失 / spawn 失败三条路径不发 task_terminated 邮件；git 路径还留永久 open 的 wakeup 行 | `scheduler.py` 三处直接 fail | 待修 |
| tick 无逐行隔离 | Phase -1/0/0.5 单个毒行（坏 recur_kind 等）每 tick 在任务派发前炸掉整个 tick | `scheduler.py` `_tick` | **已修（F2）**：phase 级守卫 + 逐行隔离 + scheduled_mail 毒行 quarantine（见 §3 异常处理粒度） |
| 共享连接回滚 | commit 锁只守 commit 边界，并发写者语句加入开放事务后可被他人 rollback 吞掉且自己 commit 空成功 | `sqlite_impl.py` `_COMMIT_LOCKS`/`transaction()` | 待修（dispatcher 独立连接方向） |
| 读回丢列 | `_row_to_task` 等不映射 created_at/updated_at/lease_until 等列，下游全见 None | `sqlite_impl.py` row mappers | 待修 |
| 压缩谎报已发 | 压缩把失败的 mailbox_send 保留为 "[Sent …]"；thrash-bail 向已回信者发"未回信"道歉且截断任务记 completed | `compact.py` preserve_out、`agent_loop.py` thrash bail | 待修 |
| thinking 跨模型 | fallback 不剥离异源 thinking 块（Anthropic 拒收）；Anthropic 从不启用 extended thinking 但 registry 标 `reasoning`；Responses reasoning 回放缺失 | `agent_loop.py` 候选分发、`adapter/` | 待修 |
| 邮箱工具越权 | mailbox_read 读他人收件箱会标对方已读（压掉 auto-wake）；cancel/list_scheduled_mail 无所有权检查 | `tools/mailbox.py` | 待修 |
| skill 加载诊断 | 碰撞/坏 frontmatter 诊断有算无报——审过的 skill 静默不加载没人知道 | `runtime/skills.py` → `context.py` 唯一调用点丢弃 diagnostics | 待修 |
| 死物 | mcp_server 空壳、skills/artifacts/local_hot 表+仓+模型、StreamError、identity 死 helper（validate_agent_id/is_bare_id）、fan_in budget_tokens/dry_round、tasks.deadline——grep 验证零调用者 | 各处 | **已删**；blocker_watcher 是 LIVE back-compat shim 保留；Config 死字段（anthropic_api_key/base_url/env_path）拆到后续配置清理批次 |
| Phase 0.7 休眠 | park/resume 保留为通用原语但无生产者 | `scheduler.py` Phase 0.7 | 保留的设计缝，非缺陷 |
...
- 2026-06-10 — 初版（PR #58）。
- 2026-06-10 — H2 无进展闸落地；fan-in 晚到结果放行 + ephemeral 终身重启上限（PR #59）。
