...
- **截断 turn 末位调用门控**：`stop_reason=max_tokens` 的 turn，其**最后一个**
...
异常处理粒度（三层）：`run()` 在 tick 级兜底；`_tick` 内 Phase -1/0.5/0/0.7/0.8
...
- **T4** 有界自邮件循环：`mailbox_send` 的 `deliver_at/deliver_in/recur_*` 参数把邮件
  ...
  （同 T4 停-loop 路径）。设计稿里的 token 预算**未实现**（`budget_tokens` 列
...
| tick 无逐行隔离 | Phase -1/0/0.5 单个毒行（坏 recur_kind 等）每 tick 在任务派发前炸掉整个 tick | `scheduler.py` `_tick` | **已修**：phase 级守卫 + 逐行隔离 + scheduled_mail 毒行 quarantine（见 §3 异常处理粒度） |
...
| 死物 | mcp_server 空壳、skills/artifacts/local_hot 表+仓、StreamError、identity 死 helper 等，grep 验证零调用者 | 各处 | 待删 |
