...
| `LYRE_LLM_MAX_RETRIES` | Transient-error retry budget passed to the provider SDK client (it retries 408/409/429/500/529 with backoff before raising). Covers the connect / first-token window; a failure *after* the first token is handled by mid-stream failover instead (`LYRE_MIDSTREAM_MAX_RETRIES`). `2` matches the SDK default; raise it for flaky providers. `0` disables SDK retry. Min clamp 0. | `2` |
| `LYRE_MIDSTREAM_MAX_RETRIES` | Per-turn budget for failing over to the **next** model candidate when a stream dies *after* it began emitting (a dropped connection / 529 mid-generation). Safe because tools dispatch only after a turn completes, so the discarded partial leaves no durable side effect. On exceed the wakeup fails as before. `0` keeps the old mid-stream-fatal behavior. Min clamp 0. | `1` |
...
| `LYRE_SINGLETON_RECOVERY_MAX` | How many times Phase-2 lease recovery silently re-runs a single **bootstrap singleton** task (the owner-facing dispatcher etc.) before failing it and escalating to the owner. The singleton is **never archived** — it must stay reachable so the next message re-wakes it. Bounds a deterministic setup-failure retry-loop that would otherwise re-run forever with nobody notified (the observed "dispatcher wakeup failed, 没人知道" bug). A transient failure that recovers within the bound never escalates. `0` disables the bound. | `3` |
